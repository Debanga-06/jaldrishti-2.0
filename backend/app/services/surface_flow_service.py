...
class SurfaceFlowService:
    ...
    async def compute_2d_surface_flow(
        ...
        rainfall_override_mm_h: Optional[float] = None,
        rainfall_source_override: Optional[str] = None,
        rainfall_provenance_override: Optional[str] = None,
    ) -> Dict[str, Any]:
        ...
        if nowcast_timestep and "rainfall_intensity_mm_h" in nowcast_timestep:
            rainfall_intensity_mm_h = float(nowcast_timestep["rainfall_intensity_mm_h"])
            rainfall_source_label = nowcast_timestep.get("source_label", "Doppler Weather Radar (DWR Optical Flow Advection)")
            rainfall_provenance = nowcast_timestep.get("provenance_label", "OBSERVED • DOPPLER WEATHER RADAR")
        elif rainfall_override_mm_h is not None:
            rainfall_intensity_mm_h = float(rainfall_override_mm_h)
            rainfall_source_label = rainfall_source_override or "Doppler Weather Radar (DWR Optical Flow Advection)"
            rainfall_provenance = rainfall_provenance_override or "OBSERVED • DOPPLER WEATHER RADAR"
        else:
