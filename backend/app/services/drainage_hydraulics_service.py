...
class DrainageHydraulicsService:
    ...
    async def evaluate_drainage_hydraulics(
        ...
        nowcast_timestep: Optional[Dict[str, Any]] = None,
    ) -> Dict[str, Any]:
        """Main coupled surface-drainage simulation pipeline for Part 4 & Part 5."""
        if surface_result:
            surface_res = surface_result
        else:
            if not nowcast_timestep:
                from app.services.nowcast_service import FloodNowcastService
                nowcast_res = await FloodNowcastService.get_0_to_3h_nowcast(
                    lat=lat, lon=lon, location_name=location_name
                )
                timesteps = nowcast_res.get("timesteps", [])
                idx = min(3, max(0, horizon_offset_hours))
                nowcast_timestep = timesteps[idx] if idx < len(timesteps) else None
        ...
        max_coupled_depth = max((c["water_depth_cm"] for c in updated_grid), default=0.0)
        avg_coupled_depth = round(sum(c["water_depth_cm"] for c in updated_grid) / max(1, len(updated_grid)), 1)

        for idx, rd in enumerate(road_summaries):
            matched_edge = edge_results[idx % len(edge_results)] if edge_results else {}
            road_factor = 1.0 if idx == 0 else 0.85
            final_max_depth = round(max_coupled_depth * road_factor, 1)
            final_avg_depth = round(avg_coupled_depth * road_factor, 1)

            if final_max_depth >= 50.0:
                risk_level = "CLOSED"
                status = "IMPASSABLE"
            elif final_max_depth >= 30.0:
                risk_level = "CRITICAL"
                status = "IMPASSABLE"
            elif final_max_depth >= 15.0:
                risk_level = "HIGH"
                status = "CAUTION"
            elif final_max_depth >= 5.0:
                risk_level = "CAUTION"
                status = "WATERLOGGED"
            else:
                risk_level = "SAFE"
            ...
            updated_road_summaries.append({
                ...
                "avg_predicted_depth_cm": final_avg_depth,
